<template>
  <v-card>
    <v-subheader>Relevant Words</v-subheader>
    <div style="height:68vh;overflow-x:hidden;overflow-y:scroll">
      <v-expansion-panels accordion multiple>
        <v-expansion-panel v-for="(key,keyIndex) in Object.keys(words)" :key="keyIndex">
          <v-expansion-panel-header>{{key}} ({{words[key].length}})</v-expansion-panel-header>
          <v-expansion-panel-content>
            <v-list>
              <v-list-item style="font-size:11px;margin-bottom:-20px" v-for="(word,index) in words[key]" :key=index>{{word.text || word}}</v-list-item>
            </v-list>
          </v-expansion-panel-content>
        </v-expansion-panel>
      </v-expansion-panels>
    ...
    methods: {
      ...
      calculateWords: function(){
        ...
        return {
          ...
          "Search": this.calculateWordsByInteractionType(interactionViewData, "SEARCH", stopWords),
        ...
        for (var i = 0; i < typeData.length; i++) {
          ...
          for (var j = 0; j < tempWords.length; j++) {
            if (!stopWords.includes(tempWords[j])) {
              if (!result.includes(tempWords[j])) {
                result.push(tempWords[j])
              }
            }
          }
        }
      ...
      calculateDocOpenWords: function (interactionViewData) {
        ...
        if(docIDs.length>0){
          ...
          return this.zipArrayMax(repVec, histograms.words, 30).sort(function(a,b){
