<template>
<v-container >
    <svg :width=totalLegendWidth height=30px v-for="(interactionLegend,interactionLegendIndex) in interactionLegendList" :key=interactionLegendIndex>
            ...
            <path v-else-if="interactionLegend.shape=='diamond'"
                transform="translate(20,16) scale(1.3)"
                :style="'fill:' + interactionLegend.color"
            ...
            <path v-else-if="interactionLegend.shape=='triangle'"
                transform="translate(20,16) scale(1.5)"
                :style="'fill:' + interactionLegend.color"
    ...
    methods: {
        resizeWithWindow: function(){        
            ...
            component.interactionLegendList.forEach(function(interactionLegend, interactionLegendIndex){
                ...
                interactionLegend.y = interactionLegendIndex<5 ? 0: 30;
                interactionLegend.x =  interactionLegendIndex < 5 ? component.width/5 * interactionLegendIndex 
                                        : component.width/5 * (interactionLegendIndex-5) ;
            });
            this.totalLegendWidth = parseFloat(component.width)/5;
        },
    },
    watch: {
        width : function(){
